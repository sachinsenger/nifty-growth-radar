    ...
    for key in keys:
        if key in data:
            ...
            if value is not None:
                return value
    # Screener section keys can be prefixed with the section heading.
...
def score(t, f):
    ...
    s += 8 if t['macd_hist'] > 0 else 2
    s += 7 if t['adx'] >= 25 else 4 if t['adx'] >= 18 else 1
    s += 7 if t['vol_ratio'] >= 1.2 else 4
    s += 5 if t['close'] > t['ema200'] else 1
    s += 5 if t['dist_high'] > -10 else 3 if t['dist_high'] > -20 else 1

    roe = first_num(f, 'Return on equity', 'ROE')
    roce = first_num(f, 'Return on capital employed', 'ROCE')
    pe = first_num(f, 'Price to Earning', 'P/E')
...
for _, x in stocks.iterrows():
    ...
    try:
        ...
        t = calculate(df)
        try:
            f = fund(str(x.screener_symbol).strip())
        except Exception as exc:
            f = {'_error': f'{type(exc).__name__}: {exc}'}

        sc = score(t, f)
        rec = 'BUY ON CONFIRMATION' if sc >= 80 else 'BUY ON PULLBACK' if sc >= 70 else 'HOLD-WATCH' if sc >= 55 else 'AVOID'
        horizon = '3-6 months' if sc >= 80 else '1-3 months' if sc >= 65 else '2-4 weeks'
        rows.append({**x.to_dict(), 't': t, 'f': f, 'score': sc, 'rec': rec, 'horizon': horizon, 'source': source})
    except Exception as exc:
        failures.append({'name': x['name'], 'symbol': symbol, 'error': f'{type(exc).__name__}: {exc}'})
