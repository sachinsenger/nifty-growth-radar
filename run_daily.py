# Enhanced daily swing radar. Generates latest + dated HTML snapshots and a machine-readable data snapshot.
import html, json
...
for _,x in stocks.iterrows():
    try:
        ...
        rows.append({**x.to_dict(),'sector':SECTORS.get(str(x.nse_symbol).strip(),'Other'),'t':t,'f':fundamentals(fraw)})
...
for r in rows:
    rs3=r['t']['ret3m']-benchmark3; vals=sec[r['sector']]; peer=[v for v in vals if v!=r['t']['ret3m']]; sec_rs=r['t']['ret3m']-(sum(peer)/max(1,len(peer))); s=score(r['t'],r['f'],market_ok,rs3,sec_rs); stop=min(r['t']['support'],r['t']['close']-1.5*r['t']['atr']); rr=1.8 if s>=85 else 1.5; target=r['t']['close']+(r['t']['close']-stop)*rr; r.update(score=s,rec=classify(r['t'],s),horizon='2-8 weeks' if s>=75 else '1-4 weeks' if s>=65 else 'observe',stop=stop,target=target,rr=rr,rs3=rs3,sector_rs=sec_rs,reasons=reasons(r['t'],r['f'],rs3,sec_rs,market_ok))
rows.sort(key=lambda r:r['score'],reverse=True)
def val(f,*k):z=first_num(f,*k);return 'N/A' if z is None else f'{z:.2f}'
today=datetime.now(timezone.utc).strftime('%Y-%m-%d')
# Persist a dated machine-readable snapshot so historical comparisons/backtesting can be built without overwriting prior data.
snapshot={'date':today,'generated_utc':datetime.now(timezone.utc).isoformat(),'market_regime':market_label,'stocks':[{k:v for k,v in r.items() if k in ('name','nse_symbol','cap','sector','score','rec','horizon','stop','target','rr','rs3','sector_rs','reasons')}|{'technical':r['t'],'fundamentals':r['f']} for r in rows],'failures':failures}
(OUT/f'data-{today}.json').write_text(json.dumps(snapshot,default=str,indent=2),encoding='utf-8')
# Keep HTML compact but decision-focused; the full data remains in JSON for reproducibility.
cards=[]
for i,r in enumerate(rows[:12],1):
 t,f=r['t'],r['f']; cards.append(f'<article class="stock"><div class="head"><div><b>#{i}</b><h3>{html.escape(str(r["name"]))}</h3><small>{r["cap"]} • {r["sector"]} • {r["horizon"]}</small></div><strong>{r["score"]}<small>/100</small></strong></div><div class="rec">{r["rec"]}</div><div class="grid"><div>Price<br><b>₹{t["close"]:.2f}</b></div><div>RSI<br><b>{t["rsi"]:.1f}</b></div><div>ADX<br><b>{t["adx"]:.1f}</b></div><div>Vol<br><b>{t["vol_ratio"]:.2f}x</b></div><div>RS vs Universe<br><b>{r["rs3"]:+.1f}pp</b></div><div>RS vs Sector<br><b>{r["sector_rs"]:+.1f}pp</b></div><div>ROE<br><b>{val(f,'Return on equity','ROE')}</b></div><div>ROCE<br><b>{val(f,'Return on capital employed','ROCE')}</b></div></div><p><b>Entry:</b> ₹{t["close"]:.2f} • <b>Stop:</b> ₹{r["stop"]:.2f} • <b>Target:</b> ₹{r["target"]:.2f} • <b>R:R:</b> 1:{r["rr"]:.1f}</p><p class="why"><b>Why:</b> {html.escape("; ".join(r["reasons"]))}</p></article>')
trs=[]
for i,r in enumerate(rows,1):
 t,f=r['t'],r['f']; trs.append(f'<tr><td>{i}</td><td>{r["cap"]}</td><td>{r["sector"]}</td><td><b>{html.escape(str(r["name"]))}</b></td><td>{r["score"]}</td><td>{r["rec"]}</td><td>{r["horizon"]}</td><td>{t["close"]:.2f}</td><td>{r["rs3"]:+.1f}%</td><td>{r["sector_rs"]:+.1f}%</td><td>{t["rsi"]:.1f}</td><td>{t["trend"]}</td><td>{t["adx"]:.1f}</td><td>{t["vol_ratio"]:.2f}x</td><td>{t["atr_pct"]:.2f}%</td><td>{val(f,'Price to Earning','P/E')}</td><td>{val(f,'Price to book','P/B')}</td><td>{val(f,'Return on equity','ROE')}</td><td>{val(f,'Return on capital employed','ROCE')}</td><td>{val(f,'Debt to equity','Debt/Eq','D/E')}</td><td>{val(f,'Sales growth','Revenue growth')}</td><td>{val(f,'Profit growth','PAT growth')}</td><td>{val(f,'EPS growth')}</td><td>{val(f,'Cash from operating activity','Operating cash flow')}</td><td>{val(f,'Free cash flow')}</td><td>{r["stop"]:.2f}</td><td>{r["target"]:.2f}</td><td>1:{r["rr"]:.1f}</td><td>{html.escape("; ".join(r["reasons"]))}</td></tr>')
page=f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Nifty Swing Growth Radar {today}</title><style>body{{font-family:system-ui;margin:0;background:#f3f6fa;color:#172033}}header{{background:#17365d;color:white;padding:22px}}main{{max-width:2200px;margin:auto;padding:14px}}.cards{{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:12px}}.card,.stock{{background:white;border-radius:15px;padding:15px;margin:12px 0;box-shadow:0 2px 9px #0001}}.head{{display:flex;justify-content:space-between}}.head h3{{margin:4px 0}}.head>strong{{font-size:30px}}.head small{{font-size:11px;color:#667085}}.rec{{font-weight:800;margin:8px 0;padding:7px;background:#eef8f0;border-radius:8px;display:inline-block}}.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}}.grid div{{background:#f7f9fb;padding:7px;border-radius:7px;font-size:11px}}.why,.note{{font-size:12px;color:#667085}}table{{border-collapse:collapse;width:100%;min-width:2800px}}th,td{{padding:7px;border-bottom:1px solid #ddd;font-size:11px;text-align:left}}th{{background:#17365d;color:white;position:sticky;top:0}}.tablewrap{{overflow:auto}}@media(max-width:600px){{.grid{{grid-template-columns:repeat(2,1fr)}}}}</style></head><body><header><h1>📈 Nifty Swing Growth Radar</h1><div>{today} • Daily 4 PM IST • Multi-factor swing screen</div></header><main><div class="cards"><div class="card"><b>Market regime</b><h2>{market_label}</h2><p class="note">NIFTY trend filter when free index data is available; otherwise breadth proxy.</p></div><div class="card"><b>Universe</b><h2>{len(rows)} stocks</h2><p class="note">{len(failures)} data failures • missing values are not fabricated.</p></div><div class="card"><b>High conviction</b><h2>{sum(r['score']>=85 for r in rows)}</h2></div><div class="card"><b>Signal philosophy</b><p class="note">Strong trend + relative strength + breakout/volume + earnings/quality + controlled entry. RSI alone never triggers BUY.</p></div></div><h2>🔥 Top swing candidates</h2><div class="cards">{''.join(cards)}</div><div class="card"><h2>Full universe</h2><div class="tablewrap"><table><thead><tr><th>#</th><th>Cap</th><th>Sector</th><th>Stock</th><th>Score</th><th>Action</th><th>Horizon</th><th>Price</th><th>RS vs U</th><th>RS vs Sector</th><th>RSI</th><th>Trend</th><th>ADX</th><th>Vol/20D</th><th>ATR%</th><th>P/E</th><th>P/B</th><th>ROE</th><th>ROCE</th><th>D/E</th><th>Sales Growth</th><th>PAT Growth</th><th>EPS Growth</th><th>CFO</th><th>FCF</th><th>Stop</th><th>Target</th><th>R:R</th><th>Why</th></tr></thead><tbody>{''.join(trs)}</tbody></table></div></div><div class="card"><p class="note">Data snapshot: <a href="data-{today}.json">data-{today}.json</a>. This is a screening system, not a guarantee of returns. Validate execution, liquidity, corporate events and slippage before trading.</p></div></main></body></html>'''
(OUT/'index.html').write_text(page,encoding='utf-8'); (OUT/f'{today}.html').write_text(page,encoding='utf-8'); pd.DataFrame(failures).to_csv(OUT/f'failures-{today}.csv',index=False); print(f'Generated latest + {today}.html + data-{today}.json; stocks={len(rows)} failures={len(failures)}')
